...
import { useFeatureToggle } from '@/contexts/FeatureToggleContext'; // Import useFeatureToggle
...
export default function Header() {
  ...
  const { isSubmissionEnabled, setIsSubmissionEnabled } = useFeatureToggle(); // Get feature toggle state
  ...
  return (
    <header className="bg-primary text-primary-foreground shadow-md sticky top-0 z-50">
      <div className="container mx-auto px-2 py-1 flex items-center justify-between gap-2 sm:gap-3">
          ...
          {loading ? (
            ...
            <div className="flex items-center gap-1.5">
              ...
              <div className="items-center gap-1 ml-1 hidden sm:flex">
                <Label htmlFor="submission-toggle" className="text-xs text-primary-foreground/80 whitespace-nowrap">
                  Submissions:
                </Label>
                <Switch
                  id="submission-toggle"
                  checked={isSubmissionEnabled}
                  onCheckedChange={setIsSubmissionEnabled}
                  className="h-4 w-8 data-[state=checked]:bg-primary data-[state=unchecked]:bg-destructive/70 [&_span]:h-3 [&_span]:w-3 [&_span]:data-[state=checked]:translate-x-4 [&_span]:data-[state=unchecked]:translate-x-0.5"
                  aria-label={isSubmissionEnabled ? "Disable user submissions" : "Enable user submissions"}
                />
              </div>
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                ...
                <DropdownMenuContent className="w-56 bg-background text-foreground" align="end" forceMount>
                  ...
                  <DropdownMenuItem asChild className="sm:hidden cursor-pointer text-xs">
                   ...
                   <DropdownMenuItem onClick={() => setIsSubmissionEnabled(!isSubmissionEnabled)} className="sm:hidden cursor-pointer text-xs">
                     {isSubmissionEnabled ? <LogOut className="mr-1.5 h-3.5 w-3.5" /> : <LogIn className="mr-1.5 h-3.5 w-3.5" />}
                     {isSubmissionEnabled ? 'Disable Submissions' : 'Enable Submissions'}
                  </DropdownMenuItem>
